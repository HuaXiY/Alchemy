...
public class GenTerrainLakes implements IGenTerrainBlocks {
	...
	public static class Builder extends InnerBuilder<Builder, GenTerrainLakes> implements IGenTerrainBuilder<GenTerrainLakes> {
		...
		public Builder grassBorderWith(IBlockState a) { grassBorderWith = a; return self(); }
		public Builder grassBorderWith(Block a) { grassBorderWith = a.getDefaultState(); return self(); }
		
		public Builder waterLakeForBiome(Biome a) {
			liquid = Blocks.WATER.getDefaultState();
			frozenLiquid = Blocks.ICE.getDefaultState();
			grassBorderWith = a.topBlock;
			return this;
		}
		
		public Builder lavaLake() {
			liquid = Blocks.LAVA.getDefaultState();
			frozenLiquid = null;
			grassBorderWith = null;
			return this;
		}
		...
		public Builder() {
			...
			liquid = Blocks.WATER.getDefaultState();
			frozenLiquid = Blocks.ICE.getDefaultState();
			grassBorderWith = Blocks.GRASS.getDefaultState();
		}
		
		@Override
		public GenTerrainLakes create() {
			return new GenTerrainLakes(amountPerChunk, liquid, frozenLiquid, grassBorderWith, minHeight, maxHeight);
		}
	}
	...
	public GenTerrainLakes(float amountPerChunk, IBlockState liquid, IBlockState frozenLiquid, IBlockState grassBorderWith,
	...
	public void genTerrainBlocks(World world, Random rand, ChunkPrimer chunkPrimer, int cx, int cz, double noiseVal) {
        ...
        if (cy < minHeight || cy > maxHeight || chunkPrimer.getBlockState(cx, cy, cz) != grassBorderWith)
        	return;
        ...
        for (x = 0; x < 16; x++)
            for (z = 0; z < 16; z++)
                for (y = 0; y < 6; y++)
                    if (cavityShape[(x * 16 + z) * 8 + y])
                    	if (y > 3)
                    		chunkPrimer.setBlockState(cx + x, cy + y, cz + z, AIR);
                    	else
                    		chunkPrimer.setBlockState(cx + x, cy + y, cz + z,
