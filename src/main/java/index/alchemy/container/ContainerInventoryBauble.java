	...
	public ItemStack transferStackInSlot(EntityPlayer player, int index) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (index == 0) { // craft output
            	...
            	for (int i = 0; i < 4; i++) {
            		SlotBauble sb = (SlotBauble) inventorySlots.get(start_id + i);
            		if (baubles.getStackInSlot(i) == null && sb.getType() == bauble.getBaubleType(item) && bauble.canEquip(item, player)) {
            			sb.inventory.setInventorySlotContents(index, item);
            			return null;
            		}
            	}
