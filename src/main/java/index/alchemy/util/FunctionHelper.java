...
public interface FunctionHelper {
	...
	@FunctionalInterface interface ExSupplier<T> { T get() throws Throwable; }
	
	static <T> Supplier<T> onThrowableSupplier(ExSupplier<T> supplier, Consumer<Throwable> handle) {
		return () -> { try { return supplier.get(); } catch (Throwable t) { handle.accept(t); return null; } };
	}
	...
	@FunctionalInterface interface ExBiFunction<A, B, C> { C apply(A a, B b) throws Throwable; }
