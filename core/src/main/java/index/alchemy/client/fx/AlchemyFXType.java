...
//        GlStateManager.disableFog();
    ...
    @SideOnly(Side.CLIENT)
    public static void registerParticle(int id, Class<?> factory) {
        Minecraft.getMinecraft().effectRenderer.registerParticle(id, $(factory, "new"));
    }
    ...
    public static void init() {
        ...
        for (Entry<FX, Class<?>> entry : fx_mapping.entrySet()) {
            ...
            try {
                Tool.setInstance(clazz, registerParticleTypes(fx.name(),
                        $.forName(clazz.getName().replace("Info", "Factory"), false), fx.ignoreRange()));
            } catch (Exception e) {
