...
public class AlchemyLaunchClassLoader extends URLClassLoader {
...
//					invalidClasses.add(name);
        ...
        try {
                ...
                if (urlConnection instanceof JarURLConnection) {
                    ...
                    if (jarFile != null && jarFile.getManifest() != null) {
                        ...
                        if (pkg == null)
                            ...
                            LogWrapper.severe("The jar file %s has a security seal for path %s, but that path is defined and not secure",
            ...
            byte clazzBytes[] = getClassBytes(untransformedName);
            if (clazzBytes == null)
                empty = true;
            final byte[] transformedClass = runTransformers(untransformedName, transformedName, clazzBytes);
            if (DEBUG_SAVE)
...
//			invalidClasses.add(name);
            if (DEBUG_FINER || !empty)
                LogManager.getLogger("LaunchWrapper").log(Level.ERROR, "Exception encountered attempting classloading: " + name, e);
    ...
    private byte[] runTransformers(final String name, final String transformedName, byte[] basicClass) {
        byte basicClassCache[] = basicClass;
        for (final IClassTransformer transformer : transformers)
            try {
                basicClass = transformer.transform(name, transformedName, basicClass);
                if (basicClassCache != null && basicClass == null) {
                ...
                if (DEBUG_FINER && !Arrays.equals(basicClassCache, basicClass)) {
                    ...
                    if (transformer instanceof ASMTransformerWrapper.TransformerWrapper)
                        wrapper = (ASMTransformerWrapper.TransformerWrapper) transformer;
                    LogWrapper.finest("%s|%s(%d->%d) is transformed by %s", name, transformedName, basicClassCache == null ?
                            -1 : basicClassCache.length, basicClass == null ? -1 : basicClass.length, wrapper == null ?
                ...
                if (transformer instanceof ASMTransformerWrapper.TransformerWrapper)
                    wrapper = (ASMTransformerWrapper.TransformerWrapper) transformer;
                LogWrapper.log(Level.ERROR, t, "On transform: %s|%s(%d), transformer: %s", name, transformedName, basicClass == null ?
...
//						resourceCache.put(name, data);
        ...
        } finally { closeSilently(classStream); }
    }
