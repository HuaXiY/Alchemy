...
public class TransformerRemote implements IClassTransformer {
    ...
    public static Class<?> createMessage(Type ownerType, Type messageType, Type args[], MethodNode method) throws ClassNotFoundException {
        ...
        types = Lists.newLinkedList();
        {
            ...
            for (Type arg : args) {
                generator.loadThis();
                fromBytes(generator, arg, name, types, cw);
                generator.putField(messageType, iterator.next().name, arg);
            }
            generator.returnValue();
            generator.endMethod();
        }
        types = Lists.newLinkedList();
        {
            ...
            for (Type arg : args) {
                generator.loadThis();
                generator.getField(messageType, iterator.next().name, arg);
                toBytes(generator, arg, name, types, cw);
            }
            generator.returnValue();
            generator.endMethod();
        }
