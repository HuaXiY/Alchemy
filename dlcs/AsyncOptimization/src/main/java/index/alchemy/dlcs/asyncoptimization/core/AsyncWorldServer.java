...
public class AsyncWorldServer extends WorldServer implements IAsyncThreadListener {
                    ...
                    world.addScheduledTask(() -> {
                        if (tickCounter % 20 == 0)
                            playerList.sendPacketToAllPlayersInDimension(
                                    new SPacketTimeUpdate(world.getTotalWorldTime(), world.getWorldTime(),
    ...
    public static class Patch$PlayerList extends PlayerList {
        ...
        @Override
        public void initializeConnectionToPlayer(NetworkManager manager, EntityPlayerMP player,
            ...
            IAsyncThreadListener.class.cast(world).syncCall(() -> {
                ...
                handler.sendPacket(new SPacketJoinGame(player.getEntityId(), player.interactionManager.getGameType(),
                        info.isHardcoreModeEnabled(), world.provider.getDimension(), world.getDifficulty(),
                        getMaxPlayers(), info.getTerrainType(), world.getGameRules().getBoolean("reducedDebugInfo")));
                handler.sendPacket(new SPacketCustomPayload("MC|Brand",
                ...
                TextComponentTranslation text = player.getName().equalsIgnoreCase(name)
                        ...
                        : new TextComponentTranslation("multiplayer.player.joined.renamed", player.getDisplayName(),
                        name);
                text.getStyle().setColor(TextFormatting.YELLOW);
                sendMessage(text);
                playerLoggedIn(player);
                handler.setPlayerLocation(player.posX, player.posY, player.posZ, player.rotationYaw,
                ...
                if (nbt != null && nbt.hasKey("RootVehicle", 10)) {
                    ...
                    Entity vehicleEntity = AnvilChunkLoader.readWorldEntity(vehicleNbt.getCompoundTag("Entity"), world,
                            true);
        ...
        public void transferPlayerToDimension(EntityPlayerMP player, int targetDimension,
                                              @Nonnull ITeleporter teleporter) {
            ...
            IAsyncThreadListener.class.cast(newWorld).syncCall(() -> {
                ...
                Collection<net.minecraft.entity.ai.attributes.IAttributeInstance> watchedAttribs = attributemap
        ...
        public EntityPlayerMP recreatePlayerEntity(EntityPlayerMP oldPlayer, int targetDimension,
            ...
            IAsyncThreadListener.class.cast(newWorld).syncCall(() -> {
                ...
                EntityPlayerMP newPlayer = new EntityPlayerMP(server, newWorld, oldPlayer.getGameProfile(),
                        interactionManager);
                ...
                if (bedPos != null) {
                    ...
                    if (spawnPos != null) {
                        newPlayer.setLocationAndAngles(spawnPos.getX() + 0.5F, spawnPos.getY() + 0.1F,
                                spawnPos.getZ() + 0.5F, 0.0F, 0.0F);
                ...
                while (!newWorld.getCollisionBoxes(newPlayer, newPlayer.getEntityBoundingBox()).isEmpty()
                ...
                newPlayer.connection.setPlayerLocation(newPlayer.posX, newPlayer.posY, newPlayer.posZ,
                ...
                newPlayer.connection.sendPacket(new SPacketSetExperience(newPlayer.experience,
                ...
                if (manager.isChannelOpen()) {
                    try {
                        ...
                        LOGGER.warn("Failed to handle packet for {}",
                                new Object[]{manager.getRemoteAddress(), exception});
    ...
    public static class Patch$DimensionManager extends DimensionManager {
        ...
        public static void unloadWorlds(Hashtable<Integer, long[]> worldTickTimes) {
            ...
            while (queueIterator.hasNext()) {
                ...
                if (dimension == null || dimension.ticksWaited < ForgeModContainer.dimensionUnloadQueueDelay
                        + InnerConfig.ticks_waited) {
    ...
    public static Hook.Result getWorldThread(FMLServerHandler handler, INetHandler netHandler) {
        return netHandler instanceof NetHandlerPlayServer
                ? new Hook.Result(((NetHandlerPlayServer) netHandler).player.world)
                : Hook.Result.VOID;
    }
    ...
    public AsyncWorldServer(MinecraftServer server, ISaveHandler saveHandler, WorldInfo info, int dimensionId,
                            Profiler profilerIn) {
